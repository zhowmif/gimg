...
impl DeflateEncoder {
        ...
        for lzss_symbol in lzss {
            match lzss_symbol {
                ...
                lzss::LzssSymbol::Backreference(dist, len) => {
                    ...
                    result.push_u16_msb(len_extra_bits, len_num_extra_bits);
                    ...
                    result.push_u16_msb(dist_extra_bits, dist_num_extra_bits);
                }
    ...
    fn generate_static_distance_table(&self) -> HashMap<u16, NewBitStream> {
        (0..30)
            .zip(0..30)
            .map(|(i, val)| (i as u16, NewBitStream::from_u32_msb(val, 5)))
            .collect()
    }

    fn generate_bitstream_from_range(start: usize, end: usize, len: u8) -> Vec<NewBitStream> {
        (start..=end)
            .map(|n| NewBitStream::from_u32_msb(n as u32, len))
            .collect()
    }
